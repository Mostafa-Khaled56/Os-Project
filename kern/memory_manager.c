...
void * create_page_table(uint32 *ptr_page_directory,
	...
	uint32 physical_address = kheap_physical_address((uint32) ptr_page_table);

	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(physical_address,
			PERM_WRITEABLE | PERM_PRESENT | PERM_USER);

	for (int idx = 0; idx < 1024; idx++) {
		ptr_page_table[idx] = 0;
	}
	tlbflush();
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = 0; j < e->page_WS_max_size; j++) {
			...
			if (va >= virtual_address
					&& va < virtual_address + (size * PAGE_SIZE)) {
		...
		get_page_table(e->env_page_directory,
				(void*) virtual_address + (i * PAGE_SIZE), &ptr_page_table);
		if (ptr_page_table != NULL) {
			for (uint32 ss = 0; ss < 1024; ss++) {
				if ((ptr_page_table[ss] & PERM_PRESENT) != 0) {
					flag = 1;
					break;
				}
			}
			if (!flag) {
				pd_clear_page_dir_entry(e, virtual_address + (i * PAGE_SIZE));
