...
struct UserProgramInfo userPrograms[] =
		{ { "fos_helloWorld", "Created by FOS team, fos@nowhere.com",
				...
				{ "tmlfq1", "Tests CPU scheduling using MLFQ", PTR_START_OF(
						tst_CPU_MLFQ_master_1) },
				{ "cpuMLFQ1Slave_1",
						"[Slave program 1] of Test tst_page_replacement_CPU_MLFQ_master_1",
						PTR_START_OF(tst_CPU_MLFQ_slave_1_1) },
				{ "cpuMLFQ1Slave_2",
						"[Slave program 2] of Test tst_page_replacement_CPU_MLFQ_master_1",
						PTR_START_OF(tst_CPU_MLFQ_slave_1_2) },
				...
				//[3] BONUSES
				{ "tr1", "tests realloc (1): normal cases", PTR_START_OF(
						tst_realloc_1) }, { "tr2",
						"tests realloc (2): special cases", PTR_START_OF(
								tst_realloc_2) },
				{ "tr3",
						"tests realloc (3): content when realloc to other location",
						PTR_START_OF(tst_realloc_3) }, { "tfs",
						"Tests freeing the unused stack pages", PTR_START_OF(
								tst_freeing_stack) }, { "ef_fib", "",
						PTR_START_OF(ef_fos_fibonacci) }, { "ef_fact", "",
						PTR_START_OF(ef_fos_factorial) }, { "ef_fos_add", "",
						PTR_START_OF(ef_fos_add) }, { "tef1", "", PTR_START_OF(
						tst_envfree1) }, { "ef_ms1", "", PTR_START_OF(
						ef_mergesort_noleakage) }, { "ef_ms2", "", PTR_START_OF(
						ef_mergesort_leakage) }, { "tef2", "", PTR_START_OF(
						tst_envfree2) },
...
void env_free(struct Env *e) {
	...
	uint32 * va = (uint32*) e->ptr_pageWorkingSet;
	unmap_frame(e->env_page_directory, (void*) va);
	for (int i = 0; i < e->page_WS_max_size; ++i) {
		va++;
		unmap_frame(e->env_page_directory, (void*) va);
	...
	for (uint32 i = 0; i < 1024; i++) {
		uint32 pa_of_page_table = e->env_page_directory[i];
		if (pa_of_page_table & PERM_PRESENT) {
			pa_of_page_table = pa_of_page_table >> 12;
			pa_of_page_table = pa_of_page_table << 12;
			struct Frame_Info* frame_of_page_table = to_frame_info(
					pa_of_page_table);
			if (PDX(USER_TOP) > i)
				free_frame(frame_of_page_table);
		}
	}

	// [4] Free the page DIRECTORY from the main memory
	struct Frame_Info* frame_of_page_directory = to_frame_info(e->env_cr3);
	free_frame(frame_of_page_directory);
