...
void placement(struct Env * curenv, uint32 fault_va)
{
	...
	if (ret != E_NO_MEM)
	{
		...
		if (ret == E_PAGE_NOT_EXIST_IN_PF)
		{
			...
			{
				panic("InVaild Access of The Address %x", fault_va);
			}
		}
...
void replacement(struct Env * curenv, uint32 fault_va)
{
	...
	int startPosition = curenv->page_last_WS_index;
	...
	while ((innerFound == 0) && (outerFound == 0))
	{
		uint32 currentPosition = curenv->page_last_WS_index;
		uint32 virtualAddress1 = env_page_ws_get_virtual_address(curenv, currentPosition);
		uint32 perm = pt_get_page_permissions(curenv, virtualAddress1);

		if (((perm & PERM_USED) == 0) && ((perm & PERM_MODIFIED) == 0))
		{
			index = curenv->page_last_WS_index;
			env_page_ws_clear_entry(curenv, index);
			victim = virtualAddress1;
		...
		if (curenv->page_last_WS_index == startPosition && outerFound == 0)
		{
			while (innerFound == 0)
			{
				uint32 cPosition = curenv->page_last_WS_index;
				uint32 virtualAddress2 = env_page_ws_get_virtual_address(curenv, cPosition);
				uint32 perm1 = pt_get_page_permissions(curenv, virtualAddress2);

				if ((perm1 & PERM_USED) == 0)
				{
					victim = virtualAddress2;
					index = curenv->page_last_WS_index;
				...
				pt_set_page_permissions(curenv, virtualAddress2, 0, PERM_USED);
				...
				if ((curenv->page_last_WS_index == startPosition) && (innerFound == 0))
				{
					break;
				}
			}
	...
	ptr_frame_info = get_frame_info(curenv->env_page_directory,(void *) victim, &page_table);
